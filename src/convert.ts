...
// ref, ref readonly, in, out, params
...
type KeywordKind = 'keyword.declarator' | 'keyword.builtin-type' | 'keyword.literal' | 'keyword.operator' | 'keyword.block-with-expr' | 'keyword.block-without-expr' | 'keyword.var' | 'keyword.method' | 'keyword.expr' | 'keyword.block-or-method' | 'keyword.method-or-other' | 'keyword.accessor' | 'keyword.other';
      ...
      switch (current.text) {
        case 'class': {
          ...
          if (genericsParamsAnnotationIndex.result) {
            tokens[genericsParamsAnnotationIndex.index].kind = 'operator.generics-params-annotation';
          ...
          if (genericsParamsAnnotationIndex.result) {
            ...
            for (let k = genericsParamsAnnotationIndex.index + 1; k < leftBraceIndex; k++) {
              ...
              if (isAtSeparator) {
                ...
                if (tokens[k].text === '@') {
                  break;
                } else if (tokens[k].text === '_' || isNext(token => token.text === ':', true, k, tokens, false, false)) {
                  break;
                } else if (tokens[k].text === '/') {
                  tokens[k].kind = 'operator.generics-params-separator';
                  isAtSeparator = true;
                } else if (tokens[k].text === 'ref' || tokens[k].text === 'params' || tokens[k].text === 'in' || tokens[k].text === 'out') {
                  break;
                } else {
                  throw new SyntaxError(tokens[k]);
                }
        ...
        case '--immut-ref-immut': {
          ...
          const rightSide = convertRightSide(tokens.slice(rightSideStartIndex.index), firstConvertedLength - 1, varType, indentLevel, changeToYieldReturn);
