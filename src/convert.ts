...
import { findLastIndex } from './parsers/functions';

export type { Token, BlockType, };
export { operators, assignmentOperators, SyntaxError, UnhandledError, indentCount, converted, convert, convertBlock, trimEmptyWords, removeEmptyWords, generateAutoGeneratedFnName, isAccessor, isNext, isLiteralCategory, isMethodKeyword, isExprKeyword };
...
const contextKeywords = new Set(['add', 'args', 'field', 'get', 'global', 'init', 'notnull', 'remove', 'set', 'value']);
          ...
          if (inheritanceIndex !== -1) {
            ...
            while (j < (atIndex === -1 ? leftBraceIndex : atIndex)) {
              ...
              if (next.text === '.') {
                ...
                currentInheritance += current.text;
                next.kind = 'operator.generics-params-annotation';
                let genericsParamsEndIndex = j;
                let isAtSeparator = true;
                const genericsParams: Token[] = [];
                for (let k = tokens.findIndex(x => x.id === next.id) + 1; k < (atIndex === -1 ? leftBraceIndex : atIndex); k++) {
          ...
          if (genericsParamsAnnotationIndex.result) {
            ...
            for (let j = genericsParamsAnnotationIndex.index + 1; j < leftBraceIndex; j++) {
              ...
              if (isAtSeparator) {
                ...
                tokens[j].kind = 'name.generics-param';
                ...
                if (tokens[j].text === '@' || tokens[j].text === '->') {
                  ...
                  tokens[j].kind = 'operator.generics-params-separator';
                  isAtSeparator = true;
            ...
            if (isAutoProp) {
              ...
              if ((removed[i].text === '=>' && removed[i + 1].text === '{') || removed[i].text === '{') {
                ...
                while (i < setEndIndex) {
                  ...
                  if (next.index === -1)
                    throw new SyntaxError(removed[i]);
                  const nextAtTokens = tokens.findIndex(s => s.id === removed[next.index].id);
                  const endAt = convertRightSide(tokens.slice(nextAtTokens), firstConvertedLength - 1, propType, true, indentLevel + 2);
                  const endAtAtRemoved = findLastIndex(removed, s => s.id <= tokens[endAt.endAt + nextAtTokens].id);
                  i = endAtAtRemoved;
