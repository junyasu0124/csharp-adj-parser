...
import { Type, convertType, parseType, searchArgsAndReturned } from './parsers/typeAndVariable';
import { convertRightSide } from './parsers/rightSide';
import { convertSwitch } from './parsers/switch';

export type { Token, BlockType, };
export { operators, SyntaxError, UnhandledError, indentCount, converted, convert, convertBlock, trimEmptyWords, removeEmptyWords, generateAutoGeneratedFnName, isAccessor, isNext, isLiteralCategory, isMethodKeyword, isExprKeyword };
...
const contextKeywords = ['add', 'args', 'field', 'get', 'global', 'init', 'notnull', 'remove', 'set', 'value'];
...
type KeywordKind = 'keyword.declarator' | 'keyword.builtin-type' | 'keyword.operator' | 'keyword.block-with-expr' | 'keyword.block-without-expr' | 'keyword.var' | 'keyword.method' | 'keyword.expr' | 'keyword.block-or-method' | 'keyword.method-or-other' | 'keyword.accessor' | 'keyword.other';
 ...
 * @returns 
 */
function convertBlock(tokens: Token[], container: BlockType, indentLevel: number, oneLine = false, changeToYieldReturn = false): number {
  type ModifiersType = {
    inheritance: ['abstract' | 'new' | 'override' | 'virtual', Token] | null,
    refImmut: ['ref' | 'immut' | 'immut ref' | 'ref immut' | 'immut ref immut', Token | { content: string, start: number, end: number }] | null,
    ...
    if (current.category === 'line_break' && i + 1 < tokens.length && tokens[i + 1].category === 'line_break') {
      ...
      switch (current.text) {
        ...
        case 'struct': {
          ...
          const indexOfUnexpected = indexOfNotBlankOrComment(structNameIndex + 1, atIndex === -1 ? leftBraceIndex : atIndex)
          if (indexOfUnexpected !== -1) {
            throw new SyntaxError(tokens[indexOfUnexpected]);
          }
          let genericsParamsEndIndex = structNameIndex;
          const genericsParams: Token[] = [];
          for (let j = structNameIndex + 1; j < leftBraceIndex; j++) {
            if (tokens[j].category !== 'space' && tokens[j].category !== 'line_break' && tokens[j].category !== 'comment') {
              if (tokens[j].text === '-') {
                tokens[j].kind = 'operator.generics-params-annotation';
                genericsParamsEndIndex = j;
          ...
          if (genericsParams.length === 0)
            converted.push(`struct ${tokens[structNameIndex].text} {\r\n`);
          else
            converted.push(`struct ${tokens[structNameIndex].text}<${genericsParams.map(x => x.text).join(', ')}> {\r\n`);
        ...
        case 'fn': {
          ...
          if (semicolonIndex !== -1 && (leftBraceIndex === -1 || semicolonIndex < leftBraceIndex)) {
            tokens[semicolonIndex].kind = 'operator.other';
            isPartial = true;
        ...
        case '--immut-ref-immut': {
          ...
          const rightSide = convertRightSide(tokens.slice(rightSideStartIndex.index), firstConvertedLength - 1, varType, indentLevel);
        ...
        case 'prop': {
          ...
          if (atMarkIndex === -1 && accessorLeftBraceIndex === -1) {
            ...
            if (nextToArrowToken.item.text === '{') {
              ...
              converted.push('get {\r\n');
              convertRightSide(tokens.slice(assignmentIndex + 1, rightSideBraceIndex), firstConvertedLength - 1, type.type, indentLevel + 1);
              converted.push('\r\n');
              converted.push(' '.repeat((indentLevel + 1) * indentCount));
              converted.push('}\r\n');
              converted.push(' '.repeat((indentLevel + 1) * indentCount));
              ...
              converted.push('=> ');
              const rightSide = convertRightSide(tokens.slice(assignmentIndex + 1), firstConvertedLength - 1, type.type, indentLevel);
            ...
            convertPropAccessor(typeEndAt + 1, assignmentIndex, i, type.type);
            ...
            const propAccessor = convertPropAccessor(typeEndAt + 1, atMarkIndex, i, type.type);
          ...
          function convertPropAccessor(start: number, end: number, propStart: number, propType: Type): { hasModifiers: boolean } {
            ...
            if (isAutoProp) {
              ...
              if ((removed[i].text === '=>' && removed[i + 1].text === '{') || removed[i].text === '{') {
                ...
                convertRightSide(removed.slice(i + 1, setIndex), firstConvertedLength - 1, propType, indentLevel + 2);
              ...
              if ((removed[i].text === '=>' && removed[i + 1].text === '{') || removed[i].text === '{') {
                ...
                convertRightSide(removed.slice(i + 1, setEndIndex), firstConvertedLength - 1, propType, indentLevel + 2);
        ...
        case 'return': {
          ...
          if (next.text === ';') {
            ...
            const endAt = convertRightSide(tokens.slice(i), converted.length - 1, null, indentLevel);
            i = endAt.endAt + i;
          }
          break;
        }
        ...
        case 'if': {
          ...
          if (container !== 'fn')
            throw new SyntaxError(current, 'If is not allowed here');

          let now: 'if' | 'elif' | 'else' = 'if';
          while (true) {
            if (now !== 'else') {
              const nextIndex = isNext(() => true, true, i, tokens, false, true) as { result: boolean, index: number };
              if (nextIndex.index === -1 || nextIndex.index === i + 1)
              ...
              const endAt = convertRightSide(tokens.slice(nextIndex.index), firstConvertedLength - 1, null, indentLevel);
              if (converted[converted.length - 1] === ';\r\n')
                converted.pop();
              converted.push(')');
              i = endAt.endAt + nextIndex.index;
              for (let j = i + 1; j < tokens.length; j++) {
                if (tokens[j].text === ';')
                  i = j;
                else
              ...
              converted.push('else');
              let nextIndex = isNext(() => true, true, i, tokens, false, true) as { result: boolean, index: number };
              if (nextIndex.index === -1)
                throw new SyntaxError(tokens[i]);
            ...
            const nextToEndIndex = isNext(() => true, true, i, tokens, false, true) as { result: boolean, index: number };
            if (nextToEndIndex.index === -1) {
              throw new SyntaxError(tokens[i]);
            }
            const nextToEnd = tokens[nextToEndIndex.index];
            if (nextToEnd.text === '{') {
              let braceCount = 1;
              let rightBraceIndex = -1;
              for (let j = nextToEndIndex.index + 1; j < tokens.length; j++) {
                if (tokens[j].text === '{') {
                  braceCount++;
                } else if (tokens[j].text === '}') {
                  braceCount--;
              ...
              if (rightBraceIndex === -1)
                throw new SyntaxError(nextToEnd, 'Missing right brace');

              converted.push(' {\r\n');
              convertBlock(tokens.slice(nextToEndIndex.index + 1, rightBraceIndex), 'fn', indentLevel + 1);
              converted.push(' '.repeat(indentLevel * indentCount));
              converted.push('}\r\n');
              ...
              converted.push('\r\n');
              const endAt = convertBlock(tokens.slice(nextToEndIndex.index), 'fn', indentLevel + 1, true);

              i = endAt + nextToEndIndex.index;
              for (let j = i + 1; j < tokens.length; j++) {
                if (tokens[j].text === ';')
                  i = j;
                else
            ...
            const nextNowIndex = isNext(() => true, true, i, tokens, false, true) as { result: boolean, index: number };
            if (nextNowIndex.index === -1)
              break;
            const nextNow = tokens[nextNowIndex.index];
            if (nextNow.text === 'elif') {
              now = 'elif';
            } else if (nextNow.text === 'else') {
              now = 'else';
            } else {
              break;
            }
            i = nextNowIndex.index;
            converted.push(' '.repeat(indentLevel * indentCount));
          }
          break;
        }
        ...
        case 'switch': {
          ...
          const endAt = convertSwitch(tokens.slice(i, rightBraceIndex + 1), converted.length - 1, null, indentLevel);
          i += endAt;
          break;
        }
        case 'enum': {
        ...
        default: {
          ...
          const rightSide = convertRightSide(tokens.slice(i), converted.length, null, indentLevel);
          i += rightSide.endAt;
          break;
        }
      }
