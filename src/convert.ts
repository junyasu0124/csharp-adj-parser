...
export { operators, SyntaxError, UnhandledError, convert, trimEmptyWords, removeEmptyWords, generateAutoGeneratedFnName, isAccessor, isNext, isLiteralCategory, isMethodKeyword };
...
type KeywordKind = 'keyword.declarator' | 'keyword.builtin-type' | 'keyword.block-with-expr' | 'keyword.block-without-expr' | 'keyword.var' | 'keyword.method' | 'keyword.expr' | 'keyword.block-or-method' | 'keyword.expr-or-method' | 'keyword.method-or-other' | 'keyword.accessor' | 'keyword.other';
...
function isMethodKeyword(kind: Kind | undefined) {
  return kind === 'keyword.method' || kind === 'keyword.block-or-method' || kind === 'keyword.expr-or-method' || kind === 'keyword.method-or-other';
}
...
type BlockType = 'none' | 'namespace' | 'class' | 'struct' | 'fn' | 'switch';
...
function convert(str: string): string {
  ...
  function convertBlock(tokens: Token[], indentLevel: number, container: BlockType, changeToYieldReturn = false) {
    const indentCount = 2;
      ...
      if (current.category === 'line_break' && i + 1 < tokens.length && tokens[i + 1].category === 'line_break') {
        ...
        switch (current.text) {
          ...
          case 'struct': {
            ...
            const block = tokens.slice(leftBraceIndex + 1, rightBraceIndex);
            convertBlock(block, indentLevel + 1, 'struct');
          ...
          case '--immut-ref-immut': {
            if (current.text === '--immut' || current.text === '--ref' || current.text === '--immut-ref' || current.text === '--ref-immut' || current.text === '--immut-ref-immut') {
            ...
            const rightSide = convertRightSide(tokens.slice(rightSideStartIndex.index), converted, convertBlock, indentLevel, indentCount);
            if (current.text === 'const') {
          ...
          case 'prop': {
            ...
            if (atMarkIndex === -1 && accessorLeftBraceIndex === -1) {
              ...
              if (nextToArrowToken.item.text === '{') {
                ...
                converted.push('get {\r\n');
                convertRightSide(tokens.slice(assignmentIndex + 1, rightSideBraceIndex), converted, convertBlock, indentLevel + 1, indentCount);
                converted.push('\r\n');
                converted.push(' '.repeat((indentLevel + 1) * indentCount));
                converted.push('}\r\n');
                converted.push(' '.repeat((indentLevel + 1) * indentCount));
                ...
                converted.push('=> ');
                const rightSide = convertRightSide(tokens.slice(assignmentIndex + 1), converted, convertBlock, indentLevel, indentCount);
            ...
            function convertPropAccessor(start: number, end: number): { hasModifiers: boolean } {
              ...
              if (isAutoProp) {
                ...
                if ((removed[i].text === '=>' && removed[i + 1].text === '{') || removed[i].text === '{') {
                  ...
                  convertRightSide(removed.slice(i + 1, setIndex), converted, convertBlock, indentLevel + 2, indentCount);
                ...
                if ((removed[i].text === '=>' && removed[i + 1].text === '{') || removed[i].text === '{') {
                  ...
                  convertRightSide(removed.slice(i + 1, setEndIndex), converted, convertBlock, indentLevel + 2, indentCount);
          ...
          case 'break': {
            const next = isNext(token => token.text === ';', true, i, tokens, true, false) as { result: boolean, item: Token | null };
            if (next.result) {
              converted.push('break;\r\n');
              i += 2;
              break;
            } else {
              throw new SyntaxError(next.item);
            }
          }
          case 'goto': {
          ...
          case 'switch': {
            const removed = removeEmptyWords(tokens.slice(i), true);
            const endAt = convertSwitch(removed, converted, null, false, i, convertBlock, indentLevel, indentCount);
            const endAtAtTokens = tokens.findIndex(s => s.id === removed[endAt - 1].id);
            if (endAtAtTokens === -1)
              throw new UnhandledError(removed[endAt - 1]);
            i = endAtAtTokens;
            break;
          }
          case 'enum': {
          ...
          default: {
            ...
            const rightSide = convertRightSide(tokens.slice(i), converted, convertBlock, indentLevel, indentCount);
            i += rightSide.endAt;
            break;
          }
        }
