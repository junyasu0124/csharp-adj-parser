import { SyntaxError, UnhandledError, Token, isNext, isMethodKeyword, operators, removeEmptyWords, isExprKeyword, convertBlock, converted, indentCount, assignmentOperators } from "../convert";
 ...
 */
function convertRightSide(tokens: Token[], insertIndex: number, assigningType: null | true | Type, shouldStatic: boolean, indentLevel: number): {
  endAt: number;
  isConst: boolean;
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      if (current.text === ';' || current.text === ',' || current.text === '{' || current.text === '}' || assignmentOperators.has(current.text)) {
        ...
        if (current.category === undefined || current.category === 'keyword' || current.category === 'context_keyword')
          isConst = false;
        else if (current.category !== 'operator' || current.kind === 'literal.bool' || current.kind === 'literal.null')
          currentMethod = true;
        converted.push(current.text);
